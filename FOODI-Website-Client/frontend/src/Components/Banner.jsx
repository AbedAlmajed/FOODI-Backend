...
//               <button type="submit" className="bg-green text-white px-4 py-2 rounded">Submit</button>
//             </div>
//           </form>
//         </div>
//       </div>
...
//       <DriverRequestModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
//     </div>
//   );
// };
  ...
  const DriverRequestModal = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        console.error('Error submitting driver request:', error.response ? error.response.data : error.message);
        Swal.fire({
          title: 'Error!',
          text: `There was an error submitting your request: ${error.response ? error.response.data.message : error.message}`,
          icon: 'error',
          confirmButtonText: 'OK'
        });
      }
