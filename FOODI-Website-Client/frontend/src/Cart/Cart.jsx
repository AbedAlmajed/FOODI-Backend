...
export default function Cart() {
  ...
  const handleQuantityChange = async (itemId, newQuantity, isCustom = false) => {
    try {
      ...
      if (newQuantity < 1) {
        ...
        // Update cart count immediately in UI
        const newTotalCount = (isCustom ? 
          customItems.reduce((sum, item) => sum + (item.customFood._id === itemId ? newQuantity : item.quantity), 0) + cartItems.reduce((sum, item) => sum + item.quantity, 0) :
          cartItems.reduce((sum, item) => sum + (item.menuItem._id === itemId ? newQuantity : item.quantity), 0) + customItems.reduce((sum, item) => sum + item.quantity, 0)
        );
  ...
  const handleRemoveItem = async (itemId, isCustom = false) => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        const token = localStorage.getItem('token');
        const response = await axios.delete(`http://localhost:4000/api/cart/remove/${itemId}?isCustom=${isCustom}`, {
          headers: { Authorization: `Bearer ${token}` },
          withCredentials: true
        });
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {customItems.map((item, index) => (
              <tr key={item.customFood._id}>
                ...
                <td className="px-4 py-2 text-sm text-gray-500">JD{(item.customFood.price * item.quantity).toFixed(2)}</td>
                <td className="px-4 py-2 text-sm font-medium">
                  <button className="text-red" onClick={() => handleRemoveItem(item.customFood._id, true)}>
                    <FaTrash className="w-4 h-4 md:w-5 md:h-5" />
                  </button>
