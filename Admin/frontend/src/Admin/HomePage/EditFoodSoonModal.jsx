...
const EditMenuItemModal = ({ isOpen, onClose, item, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`http://localhost:5000/api/food-soon/${editedItem._id}`, editedItem);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded-lg w-full max-w-md">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            <label htmlFor="imageUrl" className="block bg-white  text-sm font-medium text-gray-700 mb-1">
              Image URL
            </label>
            <input
              type="url"
