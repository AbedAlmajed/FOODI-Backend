  ...
  const toggleUserStatus = async (userId, currentStatus, userName) => {
    ...
    try {
      const result = await Swal.fire({
        ...
        text: `هل أنت متأكد أنك تريد تغيير حالة ${userName} إلى ${statusText}؟`,
        icon: 'warning',
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire(
          'تم التغيير!',
          `تم تغيير حالة ${userName} إلى ${statusText}.`,
          'success'
        );
      }
    } catch (error) {
              ...
              {currentUsers.map((user, index) => (
                <tr key={user._id} className={index % 2 === 0 ? 'bg-gray-100' : ''}>
                  ...
                  <td className="py-3 px-5 border-b">
                    <span className={`px-3 py-1 rounded-full text-xs ${user.isActive ? 'bg-green text-white' : 'bg-red text-white'}`}>
                  ...
                  <td className="py-3 px-5 border-b">
                    <button
                      ...
                      className={`mr-2 px-3 py-1 rounded ${user.isActive ? 'bg-green text-white' : 'bg-red text-white'}`}
                    >
                      {user.isActive ? 'Active' : 'Inactive'}
                    </button>
